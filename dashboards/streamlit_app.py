            ...
            return pd.DataFrame(
                {
                    ...
                    "narrative": [
                        "Sample complaint text 1",
                        "Sample complaint text 2",
                        "Sample complaint text 3",
                        "Sample complaint text 4",
                        "Sample complaint text 5",
                    ],
                }
            )
    ...
    try:
        ...
        if data_file.exists():
            ...
            if "product" not in data.columns:
                st.warning(
                    "⚠️ 'Product' column not found in data. Analytics may be limited."
                )
...
with st.sidebar:
    ...
    show_analytics = st.checkbox(
        "Show Analytics Dashboard", value=True, key="analytics_toggle"
    )
    # Auto-refresh for real-time updates
    if st.button(
        "🔄 Refresh Dashboard", key="refresh_dashboard", use_container_width=True
    ):
        st.rerun()
    ...
    st.info(
        f"**Plotly Charts:** {'✅ Available' if PLOTLY_AVAILABLE else '❌ Not Available'}"
    )
    st.info(
        f"**Imports:** {'✅ All Available' if IMPORTS_AVAILABLE else '⚠️ Demo Mode'}"
    )
    ...
    if not IMPORTS_AVAILABLE or not PLOTLY_AVAILABLE:
        ...
        st.info(
            "This is normal for cloud deployments. The app will work with reduced functionality."
        )
    ...
    if st.checkbox("🔧 Show Debug Info", key="debug_toggle"):
        ...
        if rag:
            try:
                ...
                st.info(
                    f"**Index File:** {'✅ Exists' if index_path.exists() else '❌ Missing'}"
                )
                st.info(
                    f"**Metadata File:** {'✅ Exists' if metadata_path.exists() else '❌ Missing'}"
                )
...
if data is not None:
    if "product" in data.columns:
        ...
        st.warning(
            "⚠️ Demo data loaded. For full functionality, ensure 'filtered_complaints.csv' exists in the data/ directory"
        )
    ...
    st.error(
        "❌ Failed to load data. Please check the data directory and file structure."
    )
        ...
        if "product" in data.columns:
            if PLOTLY_AVAILABLE:
                ...
                for i, (product, count) in enumerate(product_counts.items(), 1):
                    ...
                    st.markdown(
                        f"**{i}.** {product}: {count} complaints ({percentage:.1f}%)"
                    )
        else:
        ...
        if "Issue" in data.columns:
            if PLOTLY_AVAILABLE:
                ...
                for i, (issue, count) in enumerate(issue_counts.items(), 1):
                    percentage = (count / len(data)) * 100
                    st.markdown(
                        f"**{i}.** {issue}: {count} complaints ({percentage:.1f}%)"
                    )
        else:
            # Timeline chart
            if "date" in data.columns:
                if PLOTLY_AVAILABLE:
                    ...
                    st.info(
                        "Timeline chart not available without plotly. Please check the data for temporal patterns manually."
                    )
            else:
        ...
        if rag is None:
            ...
            with st.spinner("🤖 AI is analyzing your question..."):
                try:
                    ...
                    result = rag.query(
                        enhanced_prompt, st.session_state.current_product_filter
                    )
                            ...
                            with st.expander(f"📚 Sources ({len(sources)} found)"):
                                for j, source in enumerate(sources[:3]):
                                    source_text = source.get(
                                        "excerpt",
                                        source.get("text", "No content available"),
                                    )
                                    source_product = source.get(
                    ...
                    if "index" in str(e).lower() or "faiss" in str(e).lower():
                        ...
                        "memory" in str(e).lower() or "out of memory" in str(e).lower()
                    ):
...
with st.sidebar:
    if st.session_state.messages:
        ...
        if user_questions:
            ...
            with col2:
                st.success(
                    f"**🔍 Current Filter:** {st.session_state.current_product_filter}"
                )

...
def test_load_data(temp_csv_file, sample_raw_dataframe):
    ...
    with pytest.raises(
        ValueError, match="CSV file is empty or does not contain expected columns."
    ):
    ...
    with pytest.raises(ValueError, match="Missing required columns in the CSV file."):
        ...
        sample_raw_dataframe.drop(columns=["Consumer complaint narrative"]).to_csv(
            missing_col_file, index=False
        )
...
def test_clean_text():
    ...
    assert (
        clean_text("  Leading and trailing spaces.  ") == "leading and trailing spaces"
    )
...
def test_preprocess_data(sample_raw_dataframe):
    ...
    with pytest.raises(
        ValueError, match="'Consumer complaint narrative' column not found"
    ):
        preprocess_data(
            sample_raw_dataframe.drop(columns=["Consumer complaint narrative"])
        )

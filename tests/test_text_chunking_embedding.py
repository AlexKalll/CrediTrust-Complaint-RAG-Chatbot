...
def test_sample_data(sample_dataframe):
    ...
    assert set(sampled_df["Product"].unique()) == set(
        sample_dataframe["Product"].unique()
    )
...
def test_chunk_narratives(sample_dataframe):
    ...
    assert len(df_chunks) >= len(
        sample_dataframe
    )  # Chunks should be equal or more than original narratives
...
def test_create_faiss_index():
    ...
    embeddings = np.random.rand(100, 384).astype(
        "float32"
    )  # 100 embeddings of 384 dimensions

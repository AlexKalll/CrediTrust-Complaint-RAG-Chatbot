...
def test_data_files():
    ...
    if data_dir.exists():
        ...
        if complaints_file.exists():
            print(f"✓ filtered_complaints.csv exists ({complaints_file.stat().st_size / (1024*1024):.1f} MB)")
        else:
    ...
    if vectorstore_dir.exists():
        ...
        if metadata_file.exists():
            print(f"✓ metadata.parquet exists ({metadata_file.stat().st_size / 1024:.1f} KB)")
        else:
